...
class WebVid(DatasetBase):
    ...
    def _build_samples(self, samples, lock=None):
        ...
        result = []
        save_interval = 100
        for i, ex in tqdm.tqdm(enumerate(samples), desc="processing WebVid"):
            ...
            conversations = []
            for conv in ex['QA']:
                conversations.append({'from':'human', 'value': conv['i']})
                conversations.append({'from': 'gpt', 'value': conv['a']})
            # save tsv
            ...
            if not successful:
                continue
            if len(res_frames) < 2: # check video length
                continue
            new_ex = {
                'uid': f'{self.prefix}_{i}',
                'video': vpath,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ann_path', default="VideoChat2-IT/video/caption/webvid/train.json")
    parser.add_argument('--video_dir', default="webvid/videos/")
    parser.add_argument('--output_dir', default='vid_train_datasets/tsv_data/WebVid400K')
    parser.add_argument('--num_workers', type=int, default=120)
    args = parser.parse_args()

    dataset = WebVid(args.ann_path, args.video_dir, args.output_dir, args.num_workers)
    dataset.build()
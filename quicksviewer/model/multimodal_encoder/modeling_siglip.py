...
class SiglipEncoder(nn.Module):
    ...
    def forward(
        ...
        forward_n_layers = -1, # Qiji
        forward_nth_embeds = None, # Qiji
        ...
        # Qiji
        layer_start, layer_end = 0, len(self.layers)
        if forward_n_layers >0 and forward_nth_embeds is None:
            layer_end = forward_n_layers
        elif forward_n_layers >0 and forward_nth_embeds is not None:
            layer_start = forward_n_layers + 1
            inputs_embeds = forward_nth_embeds
        # for encoder_layer in self.layers:
        hidden_states = inputs_embeds
        for encoder_layer in self.layers[layer_start:layer_end]:
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        self,
        pixel_values: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        # Qiji: set to None if forward_nth_embeds are given
        if forward_nth_embeds is not None:
            pixel_values = None
            hidden_states = None
        else:
            hidden_states = self.embeddings(pixel_values, interpolate_pos_encoding=interpolate_pos_encoding)
        ...
        # Qiji: if get features of n first layers, DO NOT pass layernorm
        if forward_n_layers >0 and forward_nth_embeds is None:
            return BaseModelOutputWithPooling(
            last_hidden_state=encoder_outputs[0],
...
class SiglipVisionModel(SiglipPreTrainedModel):
    ...
    def forward(
        ...
        forward_n_layers = -1, # Qiji: add to support acquire features of first n layers.
        forward_nth_embeds = None,
    ) -> Union[Tuple, BaseModelOutputWithPooling]:
        r"""
        Params:
          @forward_n_layers: if given, only perform forward with the first n layers, and then return outputs.
          @forward_nth_embeds: if given (in this case, the `forward_n_layers` must be not -1), feed these feature to the last forward_n_layers+1 layer and perform forward in the last N-forward_n_layers layers to get outputs.
        Returns:

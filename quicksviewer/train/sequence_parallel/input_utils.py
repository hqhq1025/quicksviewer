...
def extract_local_from_list_by_video(value_list, sp_rank, sp_size, video_bound=None):
    """ Ensure video must be divided into each rank.
      @video_bound: [start_idx, end_idx)
    """
    if video_bound is None:
        video_bound = [0, len(value_list)]
    assert video_bound[0]>=0 and video_bound[1]<=len(value_list), f"Invalid video_bound: {video_bound}\n input_ids:{value_list.tolist()}"

    quotient, remainder = divmod(video_bound[1]-video_bound[0], sp_size)
    start_idx = sp_rank * quotient + min(sp_rank, remainder)
    end_idx = (sp_rank + 1) * quotient + min(sp_rank + 1, remainder)

    # Qiji: just add this
    front_off = video_bound[0]
    start_idx = start_idx+front_off if sp_rank!=0 else 0
    end_idx = end_idx+front_off if sp_rank!=sp_size-1 else len(value_list)
    return value_list[start_idx:end_idx]
...
def extract_local_input_ids_by_video(input_ids, image_positions, sp_rank, sp_size, bos_token_id=1, image_token_len=3, video_bound=None):
    """ Ensure video must be divided into each rank.
      @video_bound: [start_idx, end_idx)
    """
    if video_bound is None:
        video_bound = [0, len(image_positions)]
    assert video_bound[0]>=0 and video_bound[1]<=len(image_positions), f"Invalid video_bound: {video_bound}\n input_ids:{input_ids.tolist()}"
    ...
    front_off = video_bound[0]
    start_idx = start_idx+front_off if sp_rank!=0 else 0
    end_idx = end_idx+front_off if sp_rank!=sp_size-1 else len(image_positions)-1
...
def extract_local_position_ids_by_video(input_ids, image_positions, image_ids, sp_rank, sp_size, image_token_len=198, video_bound=None):
    if video_bound is None:
        video_bound = [0, len(image_ids)]
    assert video_bound[0]>=0 and video_bound[1]<=len(image_ids), f"Invalid video_bound: {video_bound}\n input_ids:{input_ids.tolist()}\n image_positions: {image_positions}"
    ...
    front_off = video_bound[0]
    start_idx = start_idx+front_off if sp_rank!=0 else 0
    end_idx = end_idx+front_off if sp_rank!=sp_size-1 else len(image_ids)-1
